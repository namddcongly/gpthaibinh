...
/*
 * @copyright: http://uploadify.com
 * @license: http://www.opensource.org/licenses/gpl-license.php
 * @modify by Ken Phan
 * @description: KFManager upload
 */
		...
		$.extend($.fn,{
			fileUpload:function(options) {
				if (flashVer >= 9) {
					...
					if ($.browser.msie) {
						flashElement =
						'<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="' + settings.width + '" \
							height="' + settings.height + '" id="' + $(this).attr("id")  + 'Uploader" class="fileUploaderBtn">\
						...
						flashElement = 
						'<embed src="' + settings.uploader + '?fileUploadID=' + $(this).attr("id") + data + '" quality="high" \
						width="' + settings.width + '" height="' + settings.height + '" id="' + $(this).attr("id") + 'Uploader" \
						class="fileUploaderBtn" name="' + $(this).attr("id") + 'Uploader" allowScriptAccess="' + settings.scriptAccess + '" wmode="' + settings.wmode + '" \
					...
					$(this).bind("rfuSelect", {'action': settings.onSelect}, function(event, queueID, fileObj) {
						if (event.data.action(event, queueID, fileObj) !== false) {
							...
							if (fileObj.name.length > 20) {
								var fileName = fileObj.name.substr(0,20) + '...';
							} else {
								var fileName = fileObj.name;
							}
							$('#kfmUploadList').append(KFMskins.popupUploadAppendList($(this).attr('id'), queueID, fileName, byteSize, suffix));
							$('#kfmUploadListNull').remove();
							$('#kfmUploadList tr:even').addClass('even');
							$('#kfmUploadList tr').hover(function(){
								$(this).addClass('hover');
							},function(){
					...
					$(this).bind("rfuCancel", {'action': settings.onCancel}, function(event, queueID, fileObj, data) {
						if (event.data.action(event, queueID, fileObj, data) !== false) {
							$("#" + $(this).attr('id') + queueID).remove();
						}
						var val = $('#kfmUploadList td').text();
						if(val == '') {
					...
					$(this).bind("rfuClearQueue", {'action': settings.onClearQueue}, function() {
						if (event.data.action() !== false) {
							$('#kfmUploadList').contents().fadeOut(250, function() {$('#kfmUploadList').empty()});
						}
					});
					$(this).bind("rfuError", {'action': settings.onError}, function(event, queueID, fileObj, errorObj) {
						if (event.data.action(event, queueID, fileObj, errorObj) !== false) {
							...
							$("#" + $(this).attr('id') + queueID).css({ 'background-color': '#FDE5DD'});
						}
					});
					$(this).bind("rfuProgress", {'action': settings.onProgress, 'toDisplay': settings.displayData}, function(event, queueID, fileObj, data) {
						if (event.data.action(event, queueID, fileObj, data) !== false) {
							...
							if (event.data.toDisplay == 'percentage') displayData = ' - <span style="color:#090">' + data.percentage + '%</span>';
							if (event.data.toDisplay == 'speed') displayData = ' - <span style="color:#090">' + data.speed + 'KB/s</span>';
							if (event.data.toDisplay == null) displayData = ' ';
							$("#" + $(this).attr('id') + queueID + " .percentage").html(displayData);
						}
					});
					$(this).bind("rfuComplete", {'action': settings.onComplete}, function(event, queueID, fileObj, response, data) {
						if (event.data.action(event, queueID, fileObj, unescape(response), data) !== false) {
							...
							$("#" + $(this).attr('id') + queueID + " .percentage").html(' - <span style="color:#090">Completed</span>');
						}
					});
