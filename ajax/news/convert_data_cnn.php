...
$news = $convert->getList("status=3","id desc","0,1");
...
if(count($news) > 0)
{
	...
	foreach ($news as $n)
	{
		...
		//thumb ve 300x180
		@$imageResize->createThumb(300,180,ROOT_PATH."data/temp/image.jpg",ROOT_PATH.'data/cnn_300x180/'.date('Y/n/j',$n["time_created"]).'/'.$n['img2'],true);
		//thumb ve 210x126
		@$imageResize->createThumb(210,126,ROOT_PATH."data/temp/image.jpg",ROOT_PATH.'data/cnn_210x126/'.date('Y/n/j',$n["time_created"]).'/'.$n['img2'],true);

		//lay anh hinh chu nhat 347x255 thumb ve 300x180 va 210x126
		//$tem = @file_get_contents('http://image.xahoi.com.vn:8001/news/'.date("Y/n/j", $n["time_created"]).'/'.$n["img1"]);
		...
		//thumb ve 100x100
		//@$imageResize->createThumb(100,100,ROOT_PATH."data/temp/image1.jpg",ROOT_PATH.'data/v_100x100/'.date('Y/n/j',$n["time_created"]).'/'.$n['img1']);
		//thumb ve 63x63		
		//$tem = file_get_contents('http://image.xahoi.com.vn:8001/v_63x63/'.date("Y/n/j", $n["time_created"]).'/'.$n["img1"]);		
		//file_put_contents(ROOT_PATH."data/v_63x63/".date("Y/n/j", $n["time_created"])."/".$n["img1"],$tem);
		echo date('Y/n/j',$n["time_created"]);
	}
	
	$convert->updateData(array("status"=>"4"), "","id IN($update_id)");
	
	echo '<meta http-equiv="refresh" content="2;url=ajax.php?path=news&fnc=convert_data_cnn">';
}
