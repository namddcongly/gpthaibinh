...
class FrontendNews
{
    ...
    function getNewsHome(
        $is_cache = false,
        $list_field = 'id,nw_id,censor_id,arrange,cate_id,type_post,cate_path,title,description,tag,img1,img2,img3,property,time_public,time_created,is_video',
        $wh = null
    ) {
        if ($is_cache) {
            $Cache = new CacheFile();
    ...
    function getRegionInCategory($cate_ids = '', $is_cache = false)
    {
        ...
        if ($cate_ids) {
            $wh = "cate_id IN ({$cate_ids}) AND property=1";
        } else {
            $wh = 'property =1';
        }
    ...
    {
        if ($cond == "") {
            $cond = 'time_public < ' . time();
        } else {
            $cond .= " AND time_public < " . time();
        }
        return self::$dbNews->select("store", "id AS nw_id,title,cate_id,time_created,description,time_public,img2",
            $cond, "id DESC", $limit);
    }
    ...
    function showContent(
        $content,
        $arr_search = array('https://cms.congly.com.vn', "<div", "</div>"),
        $arr_replace = array('http://congly.com.vn', "<p", "</p>")
    ) {
     ...
     * @param unknown_type $limit
     */
    function getIdNewsCate(
        $table,
        $cate_ids,
        $order = 'ORDER BY time_public DESC',
        $limit = 5,
        $is_cache = false,
        $ext = ''
    ) {
    ...
    function searchFullTextNews($keyword, $wh = '', $limit = '0,20', $accurate = false)
    {
        ...
        if ($keyword) {
            ...
            if (count($data)) {
                ...
                $result = $this->getNews('store',
                    'id,title,cate_id,img1,description,is_img,is_video,time_created,time_public',
                    "id IN ({$news_ids}) AND time_public > 0", null, null, 'id');
    ...
    function newsFocus(
        ...
        if ($is_cache) {
            $Cache = new CacheFile();
            $data = $Cache->get(md5($cate_id . $order . $limit . $wh . 'news_focus'), '', CACHE_FILE_PATH . 'news' . DS,
                300);
            if ($data) {
                return $data;
            }
        ...
        if (count($hits) > 0) {
            ...
            if ($ids != "0") {
                $data = self::$dbNews->select('store', $list_field,
                    "id IN($ids)  AND time_public > 0 AND time_public < " . time(), null, null, 'id');
                ...
                if ($is_cache) {
                    $Cache->set(md5($cate_id . $order . $limit . $wh . 'news_focus'), $_data, 300, '',
                        CACHE_FILE_PATH . 'news' . DS);
                }
                return $_data;
            }
        }
