...
class Search
{
    ...
    function index()
    {
        joc()->set_file('Search', Module::pathTemplate() . 'frontend' . DS . 'news_search.htm');
        ...
        if ($q) {
            $list_search = $frontendObj->searchFullTextNews($convert->convertUtf8ToTelex($q), $wheres, $limit,
                $accurate);
            #print_r($list_search);
        ...
        if (count($list_search)) {
            ...
            foreach ($list_search as $row) {
                if ($row["id"]) {
                    ...
                    joc()->set_var('href', Url::Link(array(
                        'cate_id' => $row['cate_id'],
                        'cate_alias' => $LIST_CATEGORY_ALIAS[$row['cate_id']],
                        'id' => $row['id'],
                        'title' => $row['title']
                    ), 'news', 'congly_detail'));
            ...
            $text_html = '<p style="margin-top: 10px; text-align: center">Không có kết quả phù hợp với từ khóa: <strong>' . str_replace(array(
                    '<',
                    '>',
                    '/'
                ), array('&lt', '&gt', ''), $q) . '</strong>.</p>';
        }
    ...
    function showIcon($row, $suffix = '')
    {
        if ($row['is_video']) {
            joc()->set_var('is_video' . $suffix,
                ' <img src="webskins/skins/news/images/video.gif"  style="float:none" />');
        } else {
            joc()->set_var('is_video' . $suffix, '');
        }
        if ($row['is_img']) {
            joc()->set_var('is_img' . $suffix,
                ' <img src="webskins/skins/news/images/image.gif"  style="float:none" />');
        } else {
            joc()->set_var('is_img' . $suffix, '');
        }
    }
